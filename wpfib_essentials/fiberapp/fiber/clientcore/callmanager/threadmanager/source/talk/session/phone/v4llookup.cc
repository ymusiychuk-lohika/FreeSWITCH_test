      ...
      if ((video_fd >= 0) || (errno == EBUSY)) {
        ...
        if ((errno == EBUSY) ||
          ...
          LOG(LS_INFO) << "Found V4L2 capture device " << device_path 
                       << " Cap bus info" << video_caps.bus_info;
          caps_bus_info = device_path.c_str();
          
          video_caps.card[sizeof(video_caps.card)-1]  = 0;
          device_display_name = (char*)video_caps.card;
          LOG(LS_INFO) << "V4L2 capture device name " << device_display_name;
