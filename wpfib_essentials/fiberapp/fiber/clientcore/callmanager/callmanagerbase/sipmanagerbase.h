...
struct MeetingFeatures {
    MeetingFeatures()
        ...
        , mVideoVividness(false)
        , mEnableInfoBar(false)
        , mForceMJPGForHD(false)
        , mImprovedTemporalScalability(false)
        , mReducedTemporalScalabilityCPU(false)
        , mRtcpXr(true)
        {}
    ...
    bool mMonotonicClockSource; // Use a monotonic clock source for outgoing AST headers
    ...
    bool mVideoVividness;
    bool mEnableInfoBar;
    bool mForceMJPGForHD;
    bool mImprovedTemporalScalability;
    bool mReducedTemporalScalabilityCPU;
    bool mRtcpXr;
};
...
struct CallTurnServerInfo
{
    relay_protocol_t   mRelayCall;
    std::string        mServer;
    std::string        mServerFqdn;
    unsigned           mTurnNodeId;
    int                mBandwidth;
    MovingAverage<int> mRttMs;
...
public:
    ...
    void PublishTurnServerInfo(pjsua_call_id call_id, int med_idx);
    relay_protocol_t getRelayCall(int med_idx) const {
        return mTurnServerInfo[med_idx].mRelayCall;
    }
    ...
    void postToClientDelayed(int msDelay, unsigned msg_id)
    {
        if (client_thread_) {
            client_thread_->Clear(this, msg_id);
            client_thread_->PostDelayed(msDelay, this, msg_id);
...
protected:
    ...
    virtual void platformPrintVolumeDetails(){ return; }
...
protected:
    ...
    CallTurnServerInfo   mTurnServerInfo[MAX_MED_IDX];
