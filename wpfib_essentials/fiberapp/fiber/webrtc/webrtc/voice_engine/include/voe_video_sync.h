...
public:
    ...
    // Sets an additional delay for the playout jitter buffer. The playout of the
    // audio is always delayed by this additional amount, |delay_ms| in milliseconds.
    // This value is added to any initial delay as well as to the delay computed
    // by NetEq's internal mechanism based on inter-arrival times and NetEq's
    // playout mode.  This extra delay is used for lip sync with video when RTX is
    // enabled.
    virtual int SetExtraPlayoutDelay(int channel, int delay_ms) = 0;
#endif
