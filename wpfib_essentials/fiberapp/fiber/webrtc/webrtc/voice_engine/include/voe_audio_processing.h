...
#include "webrtc/bjn_audio_events.h"
#endif
...
public:
    ...
    virtual int RegisterEcBufferStarvationObserver(VoeEcStarvationCallback &observer) = 0;
    virtual int DeRegisterEcBufferStarvationObserver() = 0;
    virtual int SetNsLogging(bool enable, std::string pathStr) = 0;

    // Enables logging of far-end, near-end, linear output and suppressed output signals
    virtual int SetEcAudioLogging(bool enabled, std::string pathStr) = 0;

    // Enables logging of skew, delay, erl, erle
    virtual int SetEcMetricsLogging(bool enabled, std::string pathStr) = 0;

    // Sets the Taillength of the EC
    virtual int SetEcTailLengthMs(int tailMs) = 0;

    virtual int SetEcNlpDelayFiltering(bool enable) = 0;

    virtual int SetEcDelayLogging(bool enable, const std::string& path) = 0;

    virtual int SetEcDelayMode(EcDelayModes mode, int16_t delayMs) = 0;

    virtual int SetEcModifiedBufferLogic(bool enable) = 0;

    virtual int SetBjnMuteStatus(bool muted) = 0;
    virtual int SetBjnRemoteMuteStatus(bool muted) = 0;

    virtual int RegisterAudioDeviceWarningObserver(VoEUnsupportedDeviceCallback &observer) = 0;

    virtual int DeRegisterAudioDeviceWarningObserver() = 0;

    virtual int RegisterAudioNotificationObserver(VoEAudioNotificationCallback &observer) = 0;

    virtual int DeRegisterAudioNotificationObserver() = 0;

	virtual int RegisterAudioStatsObserver(VoEAudioStatsCallback &observer) = 0;

	virtual int DeRegisterAudioStatsObserver() = 0;

    virtual int RegisterAudioMicAnimationObserver(VoEAudioMicAnimationCallback &observer) = 0;

    virtual int DeRegisterAudioMicAnimationObserver() = 0;

    virtual int SetMicAnimationState(int state) = 0;

    virtual int SetEsmStatus(bool enable) = 0;
    
    virtual int SetEsmDelayEstimatorStatus(bool enable) = 0;

    virtual int GetEsmStatus(bool& enable, EsModes &mode) = 0;
    
    virtual int GetEsmDelayEstimatorStatus(bool& enabled) = 0;

    virtual int SetEsmLogging(bool enabled, std::string pathStr) = 0;

    virtual int SetEsmMode(EsModes mode) = 0;
    
    virtual int GetEsmMode(EsModes &mode) = 0;
    
    virtual int SetEsmLevel(EsSuppression level) = 0;
    
    virtual int GetEsmLevel(EsSuppression &level) = 0;

    virtual int EnableKeyNoiseSupp(bool enable, KeynoiseSuppLevel level) = 0;

    virtual bool IsKeyNoiseSuppEnabled() = 0;

    virtual int RegisterEsmStateObserver(VoEEsmStateCallback &observer) = 0;
    
    virtual int DeRegisterEsmStateObserver() = 0;

    virtual int SetCngMode(ComfortNoiseMode mode) = 0;
 
    virtual int GetCngMode(ComfortNoiseMode &mode) = 0;

    virtual int SetAgcDigitalGainDb(int gainDb) = 0;
#endif
