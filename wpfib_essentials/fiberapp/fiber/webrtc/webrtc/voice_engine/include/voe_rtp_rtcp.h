...
public:
    ...
    // Sets the local sequence number explicitly
    virtual int SetLocalSequenceNumber(int channel, unsigned short sequenceNumber) = 0;

    // Gets the local RTP sequence number of a specified |channel|.
    virtual int GetLocalSequenceNumber(int channel, unsigned short& sequenceNumber) const = 0;

    // Sets the local starting time stamp explicitly
    virtual int SetLocalTimestamp(int channel, unsigned int timestamp) = 0;

    // Gets the local RTP timestamp of a specified |channel|.
    virtual int GetLocalTimestamp(int channel, unsigned int& timestamp) const = 0;
#endif
