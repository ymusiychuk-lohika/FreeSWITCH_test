...
public:
    ...
    // Mutes the speaker output signal completely without affecting
    // the audio device volume.
    virtual int SetOutputMute(int channel, bool enable) = 0;

    // Gets the current speaker input mute state.
    virtual int GetOutputMute(int channel, bool& enabled) = 0;

    // Speaker Volume Controls - Endpoint Volume
    virtual int32_t SetSpeakerEndpointVolume(uint32_t volume) = 0;

    virtual int32_t GetSpeakerEndpointVolume(uint32_t& volume) const = 0;

    // Speaker mute control - Endpoint control
    virtual int32_t SetSpeakerEndpointMute(bool enable) = 0;

    virtual int32_t GetSpeakerEndpointMute(bool& enabled) const = 0;
#endif
