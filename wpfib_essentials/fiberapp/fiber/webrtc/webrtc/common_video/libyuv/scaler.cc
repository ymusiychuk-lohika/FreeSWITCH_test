...
int Scaler::Scale(const RGBVideoFrame& src_frame,
                  RGBVideoFrame* dst_frame) {
  assert(dst_frame);
  if (src_frame.IsZeroSize())
    return -1;
  if (!set_)
  ...
  dst_frame->CreateEmptyFrame(dst_width_, dst_height_,
                              src_frame.get_frame_type());

  return libyuv::ARGBScale(src_frame.buffer(),
                           src_frame.stride(),
                           src_frame.width(),
                           src_frame.height(),
                           dst_frame->buffer(),
...
bool Scaler::SupportedVideoType(VideoType src_video_type,
  ...
  if ((src_video_type == kI420) || (src_video_type == kIYUV) ||
      (src_video_type == kYV12) || (src_video_type == kARGB)
      || (src_video_type == kRGB24))
    return true;
