...
public:
    ...
    virtual void setCaptureFormatRGB(bool isCaptureInRGB)
    {
        _isCaptureInRGB = isCaptureInRGB;
    }
    virtual void EnableFrameCropping(bool enable)
    {
        _isFrameCroppingEnabled = enable;
        WEBRTC_TRACE(webrtc::kTraceWarning, webrtc::kTraceVideoCapture, _id,
                            "EnableFrameCropping: %s",
                             _isFrameCroppingEnabled? "Enabled": "Disabled");
    }
    virtual bool WebRTCFrameCroppingEnabled() const
    {
        return _isFrameCroppingEnabled;
    }

    virtual void SetDesiredSize(uint32_t width, uint32_t height)
    {
        _desiredWidth = width;
        _desiredHeight = height;
    }

    virtual void EnableVideoVividness(bool enable)
    {
        _isVideoVividnessEnabled = enable;
        WEBRTC_TRACE(webrtc::kTraceWarning, webrtc::kTraceVideoCapture, _id,
            "EnableVideoVividness: %s",
            (_isVideoVividnessEnabled ? "Enabled" : "Disabled"));
    }

    virtual void SetChromaScaleFactor(int scaleFactor)
    {
        _chromaScaleFactor = scaleFactor;
        WEBRTC_TRACE(webrtc::kTraceWarning, webrtc::kTraceVideoCapture, _id,
            "SetChromaScaleFactor: %d", _chromaScaleFactor);
    }
#endif
    ...
    virtual int32_t ConfigureCaptureParams(ScreenCaptureConfig& config);
...
protected:
    ...
    int32_t DeliverCapturedFrame(BASEVideoFrame* captureFrame,
...
#ifdef BJN_WEBRTC_CHANGES
    BASEVideoFrame *_pcaptureFrame;
    bool _isFrameCroppingEnabled;
    bool _isCaptureInRGB;
    uint32_t _desiredWidth;
    uint32_t _desiredHeight;
    int32_t _previousFrameWidth;
    int32_t _previousFrameHeight;
    bool _isVideoVividnessEnabled;
    int _chromaScaleFactor;
#endif
