 ...
 public:
  ...
  class DeviceInfo {
   public:
	...
	virtual void EnableMacCameraCapability(
		bool enableMacCameraCapability) {};
  ...
  virtual int32_t ConfigureCaptureParams(ScreenCaptureConfig& config) = 0;
#ifdef BJN_WEBRTC_CHANGES
  virtual void setCaptureFormatRGB(bool isCaptureInRGB) = 0;
  virtual void EnableFrameCropping(bool enable) = 0;
  virtual bool WebRTCFrameCroppingEnabled() const = 0;
  virtual void SetDesiredSize(uint32_t width, uint32_t height) = 0;
  virtual void EnableVideoVividness(bool enable) = 0;
  virtual void SetChromaScaleFactor(int scaleFactor) = 0;
#endif
protected:
