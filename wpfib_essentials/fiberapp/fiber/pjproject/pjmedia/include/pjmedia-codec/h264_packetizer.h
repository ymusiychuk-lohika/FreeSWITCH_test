...
/**
 * Write NALs in AVCC format. This means instead of 0x00000001 start
 * code 4-byte NAL length header is appended before the payload.
 * Append an RTP payload to an H.264 picture bitstream. Note that in case of
 * noticing packet lost, application should keep calling this function with
 * payload pointer set to NULL, as the packetizer need to update its internal
 * state.
 ...
 *
 * @return		PJ_SUCCESS on success.
 * @return		PJ_EINVAL if invalid bitstream.
 * @return		PJ_ETOOSMALL if insufficient buffer.
 * @return		PJ_EIGNORED on discarding current payload.
 * @return		PJ_ENOTSUP if nal type is unsupported.
 */
PJ_DECL(pj_status_t) pjmedia_h264_unpacketize2(pjmedia_h264_packetizer *pktz,
                                               const pj_uint8_t *payload,
                                               pj_size_t   payload_len,
                                               pj_uint8_t *bits,
                                               pj_size_t   bits_len,
