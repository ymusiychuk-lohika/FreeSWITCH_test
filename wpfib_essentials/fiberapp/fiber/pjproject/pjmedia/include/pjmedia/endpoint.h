...
typedef enum pjmedia_endpt_flag
{
    ...
    , PJMEDIA_ENDPT_HAS_ULPFEC_FLAG
#if defined(PJMEDIA_HAS_RED) && (PJMEDIA_HAS_RED != 0)
    , PJMEDIA_ENDPT_HAS_RED_FLAG
#endif
    , PJMEDIA_ENDPT_HAS_RTPX_FLAG
    , PJMEDIA_ENDPT_HAS_VIDEO_DELAY_MONITORING
    , PJMEDIA_ENDPT_HAS_NEW_NACK_REQUEST
    , PJMEDIA_ENDPT_HAS_FEC_ON_UNIQUE_SSRC
    , PJMEDIA_ENDPT_REMOTE_FEC_ON_UNIQUE_SSRC
    , PJMEDIA_ENDPT_HAS_RTCP_XR
} pjmedia_endpt_flag;
 ...
 * Type of callback to register to pjmedia_endpt_atexit().
 */
typedef void (*pjmedia_endpt_bwmgr_callback)(void *usr_data,
                    unsigned idx,
                    void*pkt,
                    pj_ssize_t len,
                    pjmedia_rtp_pkt_type pkt_type);
 ...
 */
PJ_DECL(pj_ioqueue_t*) pjmedia_endpt_get_ioqueue(pjmedia_endpt *endpt, int med_idx);
...
/**
 * Returns weather the fec ssrc feature has been enabled or not
 * @param endpt		The media endpoint instance.
 *
 * @return		true if fec has unique ssrc
 */
PJ_DECL(pj_bool_t) pjmedia_endpt_has_unique_fec_ssrc(pjmedia_endpt *endpt);

/**
 * Returns weather the remote party has fec ssrc feature or not. This is
 * populated as true id the ssrc field is available in the 200ok sdp
 * @param endpt		The media endpoint instance.
 *
 * @return		true if fec has unique ssrc
 */
PJ_DECL(pj_bool_t) pjmedia_remote_support_unique_fec_ssrc(pjmedia_endpt *endpt);

/**
 * Returns the fec ssrc for incoming vi stream, i.e. camera stream
 *
 * @param endpt		The media endpoint instance.
 *
 * @return		ssrc value
 */
PJ_DECL(pj_uint32_t) pjmedia_endpt_get_fec_ssrc_inc(pjmedia_endpt *endpt);

/**
 * Returns the rtx ssrc for vi stream i.e. cam video rtx packets
 *
 * @param endpt		The media endpoint instance.
 *
 * @return		ssrc value
 */
PJ_DECL(pj_uint32_t) pjmedia_endpt_get_rtx_ssrc_inc(pjmedia_endpt *endpt);

/**
 * Generates the random ssrcs for fec for vi streams
 *
 * @param endpt		The media endpoint instance.
 */
PJ_DECL(void) pjmedia_endpt_generate_fec_ssrc(pjmedia_endpt *endpt);

/**
 * Generates the random ssrcs for rtx for vi streams
 *
 * @param endpt		The media endpoint instance.
 */
PJ_DECL(void) pjmedia_endpt_generate_rtx_ssrc(pjmedia_endpt *endpt);
 ...
 * @param pool		Pool to allocate memory from.
 * @param si		Socket information.
 * @param index	    Media index of the video channel.
 * @param p_m		Pointer to receive the created SDP media.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
...
/**
 * Set the bandwidth manager callback to monitor packet delay
 *
 * @param endpt		The media endpoint.
 * @param guid	    The endpoints globally unique identifier
 *
 * @return		PJ_SUCCESS on success.
 */
PJ_DECL(pj_status_t) pjmedia_endpt_set_bwmgr_cb(pjmedia_endpt *endpt, 
    void* data,
    pjmedia_endpt_bwmgr_callback func);

/**
 * Update the bandwidth manager callback using the callback
 *
 * @param endpt		The media endpoint.
 * @param index	    The index of the stream
 * @param recvms    The time the packet was received
 * @param pkt	    The pointer to packet data
 * @param bytes_read	    The size of the packet
 *
 * @return		PJ_SUCCESS on success.
 */
PJ_DECL(pj_status_t) pjmedia_endpt_bwmgr_update
    (pjmedia_endpt *endpt, 
     unsigned index,
     void *pkt,
