...
typedef struct pjmedia_stream_info
{
    ...
    unsigned		tx_red_pt;  /**< Outgoing FEC paylaod type.	    */
    unsigned		rx_red_pt;  /**< Incoming FEC paylaod type.	    */
#endif
...
#if defined(PJMEDIA_HAS_RED) && (PJMEDIA_HAS_RED != 0)
/**
 * Enable redundant(RED) encoding
 *
 * @param stream	The media stream.
 * @param enable	Enable/disable red encoding
 *
 * @return		PJ_SUCCESS on success.
 */
PJ_DEF(pj_status_t) 
pjmedia_stream_enable_red(
                        pjmedia_stream *stream,
                        pj_bool_t enable);

/**
 * Set red packet distance
 *
 * @param stream	The media stream.
 * @param distanceb 	distance 0 (none) to 3 (max)
 *
 * @return		PJ_SUCCESS on success.
 */
PJ_DEF(pj_status_t) 
pjmedia_stream_set_red_packet_distance(
                        pjmedia_stream *stream,
                        unsigned distance);
#endif
