...
typedef enum pjmedia_vid_frm_bit_info
{
    ...
    /**
     * The video frame is rotated
     */
    PJMEDIA_VID_FRM_ROTATED     = 2,

    /**
     * The video frame is rotated
     */
    PJMEDIA_VID_FRM_SHOULD_PROTECT = 4,

    /**
    * The video frame is incomplete
    */
    PJMEDIA_VID_FRM_INCOMPLETE = 8,
...
typedef struct pjmedia_vid_encode_opt
{
    ...
    /**
     * Flag to force the encoder to update its bitrate.
     */
    pj_bool_t   force_bitrate;
    /**
     * Bitrate to use when force_bitrate is set.
     */
    pj_uint32_t bitrate;
    /**
     * Flag to force the encoder to update its bitrate.
     */
    pj_bool_t   force_num_temporal_layers;
    /**
     * Number of temporal layers to set when force is set
     */
    pj_uint32_t num_temporal_layers;
    /**
     * Flag to force the encoder to update its rdc state.
     */
    pj_bool_t   force_rdc;
    /**
     * rdc is on or off
    ...
    pj_bool_t rdc_state;
    /**
     * Indication of loss of the channel so encoder can update its error
     * resilience scheme.
     */
    pj_uint8_t  expected_loss;
    /**
     * Flag to force the encoder to deal with packet loss.
     */
    pj_bool_t   pli_received;
    /**
     * Flag to force the encoder to deal with rpsi.
     */
    pj_bool_t   rpsi_received;
    /**
     * Flag to force the encoder to deal with slice loss.
     */
    pj_bool_t   sli_received;
    /**
     * Indication the number of long term reference frame
     */
    pj_uint32_t iLongTermRefNum;
    /**
     * Indication the picture ID of the frame with slice loss.
     */
    pj_uint32_t pictureID;
    /**
     * Indication the first MB of slice loss.
     */
    pj_uint32_t first;
    /**
     * Indication the number of MB loss in slice loss indication.
     */
    pj_uint32_t number;

    /** 
     * Flag to indicate encoder to update its size
     */
    pj_bool_t force_size;
    
    /**
     * Width and Height to use when force size is set
     */
    int       width;
    int       height;
    int       fps;
    /** 
     * Flag to indicate encoder to restart
     * currently it is used in mute/unmute case
     */
    pj_bool_t force_restart;

    /** 
     * Flag to indicate cpu load and it value.
     */
    pj_bool_t apply_cpu_action;
    int       cpu_action_type; // Action can be increase or reduce resolution.
...
typedef struct pjmedia_vid_codec_info
{
    ...
    pjmedia_format_id   dec_fmt_id[PJMEDIA_VID_CODEC_MAX_DEC_FMT_CNT];
                                        /**< Supported encoding source 
                                             format IDs                     */
    pjmedia_format_id   recd_fmt_id;    /**< Received format ID              */
...
typedef struct pjmedia_vid_codec_param
{
    ...
    pjmedia_format      recd_fmt;       /**< Received video format	    */
    ...
    pj_bool_t       tcp_relay;  /* Check if we are using tcp relay or not */

    media_index     media_idx;  /*check if the media is audio, video or content*/
    pj_uint8_t      minQP;  /* minimum quantizer */
    pj_uint8_t      maxQP;  /* maximum quantizer */
    pj_uint8_t      minQPRDC;  /* minimum quantizer for RDC(remote desktop control) */
    pj_uint8_t      maxQPRDC;  /* maximum quantizer for RDC */
    pj_uint32_t     contentHighResBitrate; /*the lowest bitrate parameter for content with 1080p resolution*/
    pj_uint32_t     videoFreeRangeQPBitrate; /*The lowest bitrate parameter for video to use free range qp.*/
} pjmedia_vid_codec_param;
