...
typedef struct pjmedia_codec_param
{
	...
	unsigned    vad:1;	    /**< Voice Activity Detector (on encode). */
        unsigned    tx_vad:1;        /**< Voice Activity Detector (on decode). */
...
typedef struct pjmedia_codec_op
{
    ...
    /**
     *  Retrieve the VAD state of the decoder.
     *
     * @param codec     The codec instante
     *
     * @return pj_bool_t VAD State: 1 if speech detected, 0 otherwise
     */
    pj_bool_t (*codec_tx_vad_state)(pjmedia_codec *codec);
} pjmedia_codec_op;
 ...
 * @return		PJ_SUCCESS on success;
 */
PJ_INLINE(pj_bool_t) pjmedia_codec_get_tx_vad( 
					pjmedia_codec *codec
					)
{
    if (codec->op && codec->op->codec_tx_vad_state)
	return (*codec->op->codec_tx_vad_state)(codec);
    else
	return 0;
}
