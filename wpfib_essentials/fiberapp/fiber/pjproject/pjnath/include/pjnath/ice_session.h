...
typedef struct pj_ice_sess_cand
{
    ...
    /**
     * Underlying Transport type of this candidate (UDP, TCP)
     */
    pj_ice_cand_tp  transport_proto;
} pj_ice_sess_cand;
...
typedef struct pj_ice_sess_options
{
    ...
    /**
     * In AWS relay server are behind private ip address for peer interface.
     * As per ICE if Xor-mapped-address is not same as local candidate prflx cansidate
     * is added for same with public ip. This flag disables adding prflx candidate
     * for relay candidates assuming its always accessible to connector (peer).
     * Disabling helps in reducing network io/traffic on AWS as connectsip will be on same box.
     */
    pj_bool_t		no_prflx_relay_candidate;
...
struct pj_ice_sess
{
    ...
    pj_ice_msg_data	 tp_data[PJ_ICE_MAX_CAND];
