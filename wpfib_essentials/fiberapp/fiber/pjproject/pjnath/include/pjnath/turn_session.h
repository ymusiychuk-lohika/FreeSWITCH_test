...
typedef struct pj_turn_address
{
    pj_sockaddr addr;
    pj_turn_tp_type tp;
} pj_turn_address_t;
...
typedef struct pj_turn_alloc_param
{
    ...
    /*
     * Indicates type of allocation transport type (UDP or TCP) used for attribute REQUESTED-TRANSPORT
     */
    pj_turn_tp_type req_tp_type;

    //Indicates the socket buffer size to be used.
    int tcp_buffer_size;

    //Call guid to be sent in allocate request to turn server
    pj_str_t call_guid;

    //Media type (audio, video or content)
    int media_type;
} pj_turn_alloc_param;
...
typedef struct pj_turn_session_info
{
    ...
    pj_bool_t	    is_proxy_enabled;

    pj_str_t	    turn_srv_fqdn;

    pj_uint32_t     turn_nodeid;
...
PJ_DECL(void) pj_turn_session_set_proxystate( pj_turn_session *sess,
					      pj_bool_t is_proxy_enabled);

PJ_DECL(void) pj_turn_session_set_srv_fqdn( pj_turn_session *sess,
					    pj_str_t *turn_srv_fqdn);
/**
