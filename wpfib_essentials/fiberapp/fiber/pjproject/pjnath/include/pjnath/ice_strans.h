...
typedef struct pj_ice_strans_cb
{
    ...
    /**
     * This callback will be called when the ICE transport async
     * send packet completes
     *
     * @param ice_st	    The ICE stream transport.
     * @param comp_id	    The component ID.
     * @param sent          Number of bytes sent.
     */
    void    (*on_data_sent)(pj_ice_strans *ice_st,
			  unsigned comp_id, 
			  pj_ssize_t sent);
...
typedef struct pj_ice_strans_turn_cfg {
    ...
    //Priority of turn server candidates.
    ...
    /**
     * Parameter to hold the address family of the default interface for turn.
     */
    int af;
...
typedef struct pj_ice_strans_cfg
{
    ...
    /**
     * DNS resolver cache to be used in conjunction with getaddrinfo
     */
    pj_dns_resolver_cache *resolver_cache;
    ...
    /* Max interval to trigger Re-Ice on no RTP. */
    int reice_interval;
     ...
     * TURN specific settings.
     */
    pj_ice_strans_turn_cfg turn[PJ_ICE_MAX_TURN];

    /**
     * The number of turn servers configured.
     */
    unsigned turn_cnt;

    /**
     * If true, always use the turn relay even if a better ice candidate
     * is available.
     */
    pj_bool_t	     turn_force;
    ...
    pj_str_t *http_proxy_host_port;
    pj_str_t *http_proxy_auth;
} pj_ice_strans_cfg;
...
PJ_DECL(unsigned) pj_ice_strans_get_turn_nodeid(pj_ice_strans *ice_st);
...
PJ_DECL(pj_status_t) pj_ice_strans_get_turn_info(const pj_ice_sess_check *chk,
