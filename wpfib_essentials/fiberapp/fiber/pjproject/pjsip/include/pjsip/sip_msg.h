...
PJ_DECL(void*) pjsip_find_remove_hdr_by_name(pjsip_hdr *start, 
				         const pj_str_t *name);
/** 
...
typedef struct pjsip_reason_hdr
{
    ...
    pj_str_t    protocol;       /** Protocol */
    int         cause;          /** Cause */
    pj_str_t    text;           /** Reason-text */
    pjsip_param other_param;    /** Other parameters */

} pjsip_reason_hdr;

/**
 * Create new Reason header instance.
 *
 * @param pool      The pool.
 *
 * @return          An empty Reason header instance.
 */
 ...
 *                  location as the mem argument.
 */
PJ_DECL(pjsip_reason_hdr*) pjsip_reason_hdr_init( pj_pool_t *pool,
                                                  void *mem );
