...
typedef struct pjsip_dlg_party
{
    ...
    pj_int32_t		 last_timedout_cseq;	    /**< Last sequence number timedout.		*/
} pjsip_dlg_party;
...
struct pjsip_dialog
{
    ...
    pj_int32_t        consecutive_req_timedout_cnt;
    pj_timer_entry    retransmit_timer;/**< Retransmit timer.     */
    pjsip_tx_data     *cloned_req;
};
...
/*
 * For the given dialog, terminate all transactions in which the endpoint is the
 * UAC (outbound requests).
 */
PJ_DECL(pj_status_t) pjsip_dlg_terminate_all_tsx_uac(pjsip_dialog *dlg);

/*
 * For the given dialog, get count of consecutive timed out failure.
 */
PJ_DECL(pj_int32_t) pjsip_get_timedout_cnt(pjsip_dialog *dlg);
