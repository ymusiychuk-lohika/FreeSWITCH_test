...
{
    /**
    * Certificate stored in char array format
    */
    pj_str_t cert_data;
    /**
				       ...
				       const pjsip_tls_setting *src)
{
    ...
    pj_strdup_with_null(pool, &dst->cert_data, &src->cert_data);
...
PJ_DECL(pj_status_t) pjsip_tls_viaproxy_transport_start(pjsip_endpoint *endpt,
					                const pjsip_tls_setting *opt,
					                const pj_sockaddr *local,
					                const pjsip_host_port *a_name,
					                unsigned async_cnt,
...
PJ_DEF(pj_status_t) pjsip_tls_transport_start_continue(pjsip_endpoint *endpt,
					               const pjsip_tls_setting *opt,
					               const pj_sockaddr *local,
					               const pjsip_host_port *a_name,
					               unsigned async_cnt,
