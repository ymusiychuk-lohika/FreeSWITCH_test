...
PJ_DECL(pj_status_t) pjsip_transport_unregister_type(int tp_type);
...
/* SKY-1943: Moved definition of pjsip_transport_state above pjsip_transport*/
/**
...
typedef enum pjsip_transport_state
{
    PJSIP_TP_STATE_CONNECTED,	    /**< Transport connected, applicable only
					 ...
					 such as TCP and TLS.		    */
    PJSIP_TP_STATE_CONNECTING,	    /**< Transport connecting, applicable only
					 to connection-oriented transports
					 such as TCP and TLS.		    */
...
struct pjsip_transport
{
    ...
    pjsip_transport_state tp_state; /** SKY-1943: storing transport state to check while acquiring transport */
...
PJ_DECL(pj_status_t) pjsip_tpmgr_acquire_transport3(pjsip_tpmgr *mgr,
						    pjsip_transport_type_e type,
						    const pj_sockaddr_t *remote,
						    int addr_len,
						    const pjsip_tpselector *sel,
 ...
 * @return	    PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DEF(pj_status_t) pjsip_transport_clean(pjsip_tpmgr *mgr);
