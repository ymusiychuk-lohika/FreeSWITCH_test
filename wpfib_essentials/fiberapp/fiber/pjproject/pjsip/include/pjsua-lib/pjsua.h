...
/**
 * Maximum TURN servers in account.
 */
#ifndef PJSUA_ACC_MAX_TURN_SERVERS
#   define PJSUA_ACC_MAX_TURN_SERVERS    8
#endif

#ifndef PJSUA_HTTP_PROXY_MAX_CRED_LENGTH
#   define PJSUA_HTTP_PROXY_MAX_DOMAIN_LENGTH 256
#   define PJSUA_HTTP_PROXY_MAX_UNAME_LENGTH 128
#   define PJSUA_HTTP_PROXY_MAX_PWD_LENGTH 128
#   define PJSUA_HTTP_PROXY_MAX_CRED_LENGTH (PJSUA_HTTP_PROXY_MAX_DOMAIN_LENGTH + PJSUA_HTTP_PROXY_MAX_UNAME_LENGTH + PJSUA_HTTP_PROXY_MAX_PWD_LENGTH - 1)
#endif
...
typedef struct pjsua_media_turn_server_per_media_config {
    /**
     * Force TURN relay candidate to be used.
     */
    pj_bool_t force_turn;
    pj_bool_t force_turn_tcp;
...
}pjsua_media_turn_server_per_media_config;

typedef struct pjsua_media_turn_server_config {
    /**
    ...
    /*
     * Per media turn server config; 
     */
    pjsua_media_turn_server_per_media_config per_media_cfg[PJMEDIA_MAX_SDP_MEDIA];

    /*
     * Certificate stored in char array format will be used for TLS connections only.
     */
    pj_str_t trusted_ca_cert_list;

    //Priority of this turn server candidate.
    pj_uint16_t priority;

    //Bandwidth used by a call if a turn server is selected. 
    int	    bandwidth;
} pjsua_media_turn_server_config;

typedef struct http_proxy_auth_info
{
    char dom_uname_pwd[PJSUA_HTTP_PROXY_MAX_DOMAIN_LENGTH + 
	   		    PJSUA_HTTP_PROXY_MAX_UNAME_LENGTH +
			    PJSUA_HTTP_PROXY_MAX_PWD_LENGTH];
} http_proxy_auth_info;
...
typedef struct pjsua_call_setting
{
    ...
    /**
     * Number of TURN servers specified.
     */
    unsigned		turn_count;
     ...
     */
    pjsua_media_turn_server_config turn[PJSUA_ACC_MAX_TURN_SERVERS];
    
    http_proxy_auth_info http_proxy_auth;
...
typedef struct pjsua_callback
{
     ...
     *
     * @param call_id	    Call identification.
     * @param call_med	    Call's media stream.
...
typedef struct pjsua_config
{
    ...
    /**
     * User data to be passed intially when transport_state_callback is invoked
     * but can be used with multiple callbacks when we need.
     */
    void             *user_data;
...
/**
 * cleanup pending transports
 *
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */

PJ_DECL(pj_status_t) pjsua_transport_clean();
...
typedef struct pjsua_acc_config
{
     ...
     * If inbuilt preview is not supported by capturing device
     *
     * Default: PJMEDIA_VID_DEFAULT_RENDER_DEV
     */
    pjmedia_vid_dev_index vid_cap_rend_dev;
...
/*
 * Get user data associated with pjsua config
 */
PJ_DECL(void *) pjsua_get_user_data(void);

/*
* Set ipv6 active/inactive flag in pjsua config
*/
PJ_DECL(void) pjsua_set_ipv6_state(pj_bool_t is_active);

/*
 * Get ipv6 active/inactive flag in pjsua config
 */
PJ_DECL(pj_bool_t) pjsua_get_ipv6_state(void);
...
typedef enum pjsua_vid_req_keyframe_method
{
    ...
    /**
     * Requesting keyframe via Fast Intra Request of RTCP feedback.
     * This is currently not supported.
     */
    PJSUA_VID_REQ_KEYFRAME_RTCP_FIR	= 4
...
typedef struct pjsua_call_media_info
{
    ...
    /* indicate if ice negotiation complete */
    pj_bool_t ice_complete;
...
typedef struct pjsua_stream_stat
{
    ...
    /**< Number of frames received from capturer. */
    pj_uint32_t         enc_frames_from_capturer;

    /**< Number of frames handed to the encoder. */
    pj_uint32_t         enc_frames_pre_encode;
 ...
 * Capture device view.
 */
typedef struct pjsua_cap_dev_view
{
    /** Capture screen */
    pj_int32_t      screen_index;

    /** Capture PID */
    pj_uint32_t     app_pid;

    /** Windows only: Screen capture using desktop duplication api */
    pj_bool_t      win_screen_capture_using_dd;

    /** Mac OS X 10.8+ only: Deferred screen capture */
    pj_bool_t      mac_screen_capture_deferred;
    pj_bool_t      mac_screen_capture_delta;

    /** Windows 8 & above only: Screen/app capture using desktop duplication api */
    pj_bool_t      win_capture_using_dd_complete;

    /** Windows 7 & above: App sharing with Global hook (Privacy strengthening) */
    pj_bool_t      win_app_capture_global_hooks;

    /** Mac and Windows enable sharing border info bar when screen sharing */
    pj_bool_t      enable_info_bar;
...
typedef enum pjsua_call_vid_strm_op
{
     ...
     * Change capture device of a video stream.  This will not send
     * re-INVITE or UPDATE to remote party.
     * see pjsua_cap_dev_view for paramater details
     */
    PJSUA_CALL_VID_SCREEN_SHARE_SET_VIEW,
    
    /**
     * Start receving video stream. This will cause previously
     * stopped stream to start receving again.
     */
    PJSUA_CALL_VID_STRM_START_RECEIVE,
    /**
     * Stop receving video stream. This will cause previously
     * started stream to stop receving.
     */
    PJSUA_CALL_VID_STRM_STOP_RECEIVE,
    /**
    ...
    /** 
     * Sennd a tmmbr. This will generate an RTCP AVPF feedback message.
     */
    PJSUA_CALL_VID_STRM_SEND_TMMBR,
    
    /** 
     * Sennd a tmmbn. This will generate an RTCP AVPF feedback message.
     */
    PJSUA_CALL_VID_STRM_SEND_TMMBN,
    
    /**
     * Enable RTX.
     */
    PJSUA_CALL_VID_ENABLE_RTX,
    
    /**
     * Disable RTX.
     */
    PJSUA_CALL_VID_DISABLE_RTX,

    /**
     * Adjust RTX delay.
     */
    PJSUA_CALL_VID_SET_RTX_DELAY,

    /**
     * Set the outbound loss rate.
     */
    PJSUA_CALL_VID_SET_OUTBOUND_LOSS_RATE,

    /**
     * Set the outbound loss rate.
     */
    PJSUA_CALL_VID_SET_NUM_NACK_RETRANSMITS,
...
typedef struct pjsua_call_vid_strm_op_param
{
    ...
    /**
     * Specify the video render device ID. This can be set to
     * PJMEDIA_VID_DEFAULT_RENDER_DEV to specify the default render
     * device as configured in the account.
     *
     * Default: PJMEDIA_VID_DEFAULT_RENDER_DEV.
     */
    pjmedia_vid_dev_index rend_dev;
    
    /** 
     * Specify the bitrate for tmmbr
     */
    int bitrate;
    
    /** 
     * Specify the overhead for tmmbr
     */
    int overhead;

    /* specify extra info eg: screen share view/frame ptr */
    void *data;

    /*Boolean to indicated while modifying video initiate reIce*/
    pj_bool_t reice;
...
/**
 * Get pjsua call attached to the call id
 *
 * @param call_id	Call identification.
 *
 * @return		The pjsua_call.
 */
PJ_DECL(void *) pjsua_call_get_call(pjsua_call_id call_id);
...
/* Difference from pjsua_call_reinvite2 is that this function calls
 *  apply_call_setting1 that unconditionally performs media channel reinit
 */
PJ_DECL(pj_status_t) pjsua_call_reinvite3(pjsua_call_id call_id,
					  const pjsua_call_setting *opt,
					  const pjsua_msg_data *msg_data);
...
/**
 * Determine if video stream for the specified call is currently created 
 *
 * @param call_id   Call identification.
 * @param med_idx   Media stream index, or -1 to specify default video
 *          media.
 *
 * @return      PJ_TRUE if stream is currently created 
 */
PJ_DECL(pj_bool_t) pjsua_call_vid_stream_is_created(pjsua_call_id call_id,
                                                    int med_idx);
...
struct pjsua_media_config
{
    ...
    /**
     * Specify whether reference count should be enabled.
     */
    pj_bool_t vid_preview_enable_reference_count;

    /* Max time to wait to trigger re-ice*/
    int reice_interval;
};
...
/**
 * Structure to pass when initiating fast change of devices
 */
typedef struct pjsua_fast_dev_change_info
{
    /**
     * Stream direction.
     */
    ...
    /**
     * Index of the playback device.
     */
    pjmedia_aud_dev_index play_id;

} pjsua_fast_dev_change_info;
...
/**
 * Pause the currently selected sound device
 *
 * @return      PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_pause_snd_dev(void);
...
PJ_DECL(pj_status_t) pjsua_snd_get_setting(pjmedia_aud_dev_cap cap,
					   void *pval);
/* 
 * Api to start/stop transmitting of audio packets in a call
 *
 * @param call_id: call id of call
 * @param med_idx: media index of audio stream
 * @param enable: indicates to start or stop of transmission
 * @return PJ_SUCCESS on success or the error code. 
 */
PJ_DECL(pj_status_t) call_set_tx_audio(pjsua_call_id call_id, int med_idx, pj_bool_t enable);
...
/**
 * Set wheter recording/playback device used are internal or external.
 * Used for delay estimation for echo. This api is called just before pjsua_set_snd_dev.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_snd_set_internal_device_param(pj_bool_t rec_id_internal, pj_bool_t play_id_internal);

/**
 * Hack to pass config parser object to BJN Webrtc Audio implementation
 * We will set this in pjsua_var.param. And which will pass it to create stream when creating param.
 * Assumption is pointer stored here is always valid whenever we call create stream
 * @return	        Always returns PJ_SUCCESS
 */
PJ_DECL(pj_status_t) pjsua_snd_set_config_parser(void* config_parser);

/**
 * Save the sequence number and timestamp of the RTP packets
 * @param call_med	    Call's media stream.
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_save_seq_num_ts(pjsua_call_id call_id, int med_idx);
...
/**
 * Get pjmedia_vid_dev_stream.
 *
 * @param wid		The video window ID.
 * @param capture   Wheter to initialize capture or rendering stream .
 * @param w		    The video stream to be initialized.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DEF(pj_status_t) pjmedia_vid_dev_stream_get( pjsua_vid_win_id wid, int capture,
                                                pjmedia_vid_dev_stream **vid_stream);

/**
 * Update capture device index for preview window. 
 * This will be called when new device will be inserted on removed while in call
 *
 * @param wid		 The video window ID.
 * @param new_cap_id New index of capture id
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DEF(pj_status_t) pjmedia_vid_dev_update_cap_id(pjsua_vid_win_id wid,
                                                  pjmedia_vid_dev_index new_cap_id);
...
/**
* Rotate the camera, This function will change the camera angle
* @param wid     The camera window ID
* @parame angle  The rotation angle in degrees.
* @return  PJ_SUCCESS on success, or the appropriate error code.
*/
PJ_DEF(pj_status_t) pjsua_cam_rotate(pjsua_vid_win_id wid,
                                     int angle);
 ...
 *
 * @param call_id	The call ID.
 * @param media_idx The stream index
 * @param bitrate	Bitrate to be applied to the video encoder
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_media_apply_video_bitrate(pjsua_call_id call_id,
                                            int med_idx,
                                            unsigned bitrate);
 ...
 * @param call_id	The call ID.
 * @param media_idx The stream index
 * @param width 	Width to be applied to the video encoder
 * @param hieght 	Hieght to be applied to the video encoder
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_media_apply_video_size(pjsua_call_id call_id,
                                            int med_idx,
 ...
 * @param call_id	The call ID.
 * @param media_idx The stream index
 * @param layers	Number of temporal layers from 0 (none) to 4
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_media_apply_video_temporal_layers(pjsua_call_id call_id,
                                            int med_idx,                 
                                            unsigned layers);
/**
 ...
 * @param call_id	The call ID.
 * @param media_idx The stream index
 * @param bitrate	Bitrate which min of configured, sdp and tmmbr
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_uint32_t) pjsua_media_vid_enc_bitrate(
                                                 pjsua_call_id call_id,
                                                 int med_idx);
/**
 * Enable video forward error correction (fec)
 *
 * @param call_id	The call ID.
 * @param media_idx The stream index
 * @param bitrate	Bitrate to be applied to the video encoder
 ...
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_media_enable_video_fec(pjsua_call_id call_id,
                                            int med_idx,       
                                            pj_bool_t enable);


/**
 * Enable redundant (red) audio 
 *
 * @param call_id	The call ID.
 * @param bitrate	Bitrate to be applied to the video encoder
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_media_enable_red(pjsua_call_id call_id,
                                            pj_bool_t enable);
 ...
 *
 * @param call_id	The call ID.
 * @param distance	packet distance from 0 (off) to 3 (max)
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_media_apply_red_packet_distance(pjsua_call_id call_id,
                                            unsigned distance);

/**
 * Stop video stream of a call
 *
 * @param call_id	The call ID.
 * This is required to stop camera capture completely and
 * To avoid sending reinvite unnecessarily sometimes 
 */
void pjsua_stop_video_stream(pjsua_call_id call_id, pjmedia_dir direction, int med_idx);
pj_status_t pjsua_video_start_stream(pjsua_call_id call_id, pjmedia_dir direction, int med_idx, const pj_str_t *enc_fmt, const pj_str_t *dec_fmt);
pj_status_t pjsua_video_restart_stream(pjsua_call_id call_id, pjmedia_dir direction, int med_idx, const pj_str_t *enc_fmt, const pj_str_t *dec_fmt);

/**
 * Enable/disable yuv capture
 *
 * @param call_id	The call ID.
 * @param enable	bool to decide if enable or diable yuv capture
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_enable_yuv_capture(pjsua_call_id call_id,
                                              pj_bool_t enable,
                                              int med_idx,
                                              const char *filePath);

void pjsua_set_vid_rc_method(pjsua_call_id call_id, 
                             pjmedia_vid_stream_rc_method method,
                             unsigned bandwidth);
void pjsua_set_vid_stream_rc_method(pjsua_call_id call_id, 
                             pjmedia_vid_stream_rc_method method,
                             unsigned bandwidth, 
                             int med_idx);
pj_status_t pjsua_vid_channel_dev_init(pjsua_call_id call_id,
                                       int med_idx,
                                       pjmedia_vid_dev_index vid_cap_dev,
                                       pjmedia_vid_dev_index vid_rend_cap_dev,
                                       pjmedia_vid_dev_index vid_rend_dev);
pj_status_t pjsua_vid_channel_codec_update(pjsua_call_id call_id, 
                                           int med_idx,
                                           int width,
                                           int height,
                                           int fps);

PJ_DECL(pj_status_t) pjsua_mute_speaker(pj_bool_t val);
PJ_DECL(pj_bool_t) pjsua_is_aud_dev_interrupted();

typedef struct pjsua_webrtc_voe_info {
    pjmedia_stream_info *si;
    struct pjsua_call_media *call_med;
} pjsua_webrtc_voe_info;

//Funtion to check if RTCP messages are received in timout period of SIP messages.
PJ_DECL(pj_bool_t) on_media_status(pjsip_inv_session *inv,  pjsip_event *e);

/**
 *  Update a custom BJN stat.  As an implementation detail, this utilizes the video
 *  channel.  Should be reasonable tolerant of multithreaded calls -- obviously the
 *  call and the underlying streams need to remain valid.  Toggling between
 *  float and int for a given identifier may be problematic.
 *
 * @param call_id	The call identification.
 * @param psi		To be filled with the stream statistic.
 *
 * @return		PJ_SUCCESS on success or the appropriate error.
 */
PJ_DECL(pj_status_t) pjsua_update_bjn_stat(pjsua_call_id call_id,
                                           const bjn_stat_element *stat);

/**
 *  Update a set of custom BJN stats.  As an implementation detail, this utilizes the video
 *  channel.  Should be reasonable tolerant of multithreaded calls -- obviously the
 *  call and the underlying streams need to remain valid.  Toggling between
 *  float and int for a given identifier may be problematic.
 *
 ...
 * @return		PJ_SUCCESS on success or the appropriate error.
 */
PJ_DECL(pj_status_t) pjsua_update_bjn_stats(pjsua_call_id call_id,
                                           const bjn_stat_element *stats,
                                           pj_size_t len);

/**
 *  Sends a custom BJN stat, but only for a couple times.  As an implementation detail, this utilizes the video
 *  channel.  Somewhat less tolerant of multithreaded calls in that a small
 *  race condition exists between expiring out and a setting a new event (it means
 *  the event could get lost)
 *
 * @param call_id	The call identification.
 * @param psi		To be filled with the stream statistic.
 *
 * @return		PJ_SUCCESS on success or the appropriate error.
 */
PJ_DECL(pj_status_t) pjsua_send_bjn_stat_oneshot(pjsua_call_id call_id,
                                           const bjn_stat_element *stat);


/**
 *  Force decoder to generate key frame request via FIR RTCP 
 * @param call_id	The call identification.
 * @param med_idx	Media index in on which request should be generated.
 *
 * @return		PJ_SUCCESS on success or the appropriate error.
 */
PJ_DECL(pj_status_t) pjsua_vid_request_key_frame(pjsua_call_id call_id, int med_idx);

/**
 * Apply CPU load strength
 *
 * @param call_id	The call ID.
 * @param media_idx The stream index
 * @param strength  CPU load in %
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_media_apply_cpu_action(pjsua_call_id call_id, int med_idx, int cpu_action_type);
