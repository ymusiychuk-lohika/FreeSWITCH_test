...
struct pjsua_call_media
{
	...
	struct {
        ...
        pjmedia_vid_dev_index rdr_cap_dev;  /**< The video-out render device */
    ...
    pj_bool_t       skip_media_update; /**skip media re initialization in reinvite. */
};
...
struct pjsua_data
{
    ...
    pj_dns_resolver_cache resolver_cache; /**< DNS revoler cache  */
...
pj_status_t pjsua_media_channel_deinit_idx(pjsua_call_media *call_med);
pj_status_t pjsua_media_channel_deinit_med_idx(pjsua_call_id call_id, int mi);
...
void apply_turn_settings(const pjsua_call_setting *opt);
...
void pjsua_aud_remove_slot(pjsua_call_media *call_med);
void pjsua_aud_add_slot(pjsua_call_media *call_med);
