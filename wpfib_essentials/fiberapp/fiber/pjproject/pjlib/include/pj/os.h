 ...
 */
PJ_DECL(pj_status_t) pj_thread_micro_sleep(pj_uint32_t usec);

/**
 * Put the current thread to sleep until the specified time.
 * Minimum sleep is sleep(0), if specified time has already passed.
 *
 * @param ts	    High resolution timer value.
 *
 * @return zero if successful.
 */
PJ_DECL(pj_status_t) pj_thread_sleep_until(pj_timestamp *ts);
...
/* BJN added atomic wrappers */

/* these are pj_direct_atomic_exchange_<type> functions.
 * These all set *dst to val and return old contents of *dst.
 */
#if _MSC_VER
PJ_INLINE(pj_uint8_t) pj_direct_atomic_exchange_u8(volatile pj_uint8_t *dst, pj_uint8_t val)
{
    return _InterlockedExchange8((volatile char *)dst, val);
}

PJ_INLINE(pj_uint32_t) pj_direct_atomic_exchange_s32(volatile pj_int32_t *dst, pj_int32_t val)
{
    return _InterlockedExchange((volatile long *)dst, val);
}

PJ_INLINE(pj_uint32_t) pj_direct_atomic_exchange_u32(volatile pj_uint32_t *dst, pj_uint32_t val)
{
    return _InterlockedExchange((volatile long *)dst, val);
}

/* essentially returns ++val */
PJ_INLINE(pj_uint32_t) pj_direct_atomic_increment_u32(volatile pj_uint32_t *val)
{
    return _InterlockedIncrement((volatile long *)val);
}

/* essentially returns --val */
PJ_INLINE(pj_uint32_t) pj_direct_atomic_decrement_u32(volatile pj_uint32_t *val)
{
    return _InterlockedDecrement((volatile long *)val);
}

#elif defined(__clang__) || defined(__GNUC__)
#define pj_direct_atomic_exchange_u8   __sync_lock_test_and_set
#define pj_direct_atomic_exchange_s32  __sync_lock_test_and_set
#define pj_direct_atomic_exchange_u32  __sync_lock_test_and_set
#define pj_direct_atomic_increment_u32(val) __sync_add_and_fetch((val), 1)
#define pj_direct_atomic_decrement_u32(val) __sync_sub_and_fetch((val), 1)

#else
#error "need to define atomics"
#endif

#if _MSC_VER
#define pj_memory_barrier _ReadWriteBarrier
#elif defined(__clang__) || defined(__GNUC__)
#define pj_memory_barrier __sync_synchronize
#else
#error "No known memory barrier wrapper.  Evaluate if this is an issue or not"
#endif
...
/**
 * Timed wait for event object to be signalled.
 *
 * @param event The event object.
 ...
 * @return zero if successfull.
 */
PJ_DECL(pj_status_t) pj_event_timedwait(pj_event_t *event, unsigned millisec);
