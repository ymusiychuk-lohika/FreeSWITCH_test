...
#define RES_HASH_TBL_SIZE   31
typedef struct pj_dns_resolver_cache
{
    pj_hash_table_t *cache_tbl;
    pj_pool_t *pool;
} pj_dns_resolver_cache;
typedef pj_hash_table_t pj_resolver_cache_tbl;
...
/**
 * This function fills the structure of type pj_hostent for a given host name.
 * This function will work for both ipv4 and ipv6 queries.`
 *
 ...
 * @return	    PJ_SUCCESS, or the appropriate error codes.
 */ 
PJ_DECL(pj_status_t) pj_gethostbyname2(const pj_str_t *name, pj_hostent *he, int af);
...
PJ_DECL(pj_status_t) pj_getaddrinfo_cache(int af, const pj_str_t *name,
				    unsigned *count, pj_addrinfo ai[], pj_resolver_cache_tbl *cache_tbl, 
                    pj_pool_t *pool);

PJ_DECL(pj_status_t) pj_flush_resolver_cache(pj_resolver_cache_tbl *cache_tbl);
PJ_DECL(pj_status_t) pj_rotate_resolver_cache(pj_resolver_cache_tbl *cache_tbl);
PJ_DECL(pj_status_t) pj_update_resolver_cache(const pj_str_t *nodename, unsigned count,
                    pj_addrinfo ai[], pj_resolver_cache_tbl *cache_tbl, pj_pool_t *pool);
PJ_DECL(pj_status_t) pj_lookup_resolver_cache(pj_resolver_cache_tbl *cache_tbl, 
                    const pj_str_t *name, pj_addrinfo *ai, unsigned *count);

PJ_DECL(pj_bool_t) pj_has_ipv4_interface();
PJ_DECL(pj_bool_t) pj_has_ipv6_interface();
